...
const replace = (oldThing, newThing) => {
  ...
  logger.warn(
    chalk.yellow(
      `Option ${chalk.bold(
        `"${oldThing}"`
      )} has been removed, and replaced by ${chalk.bold(`"${newThing}"`)}`
    ),
    '\n'
  )
  logger.warn(
    chalk.yellow(
      'This option will be removed in the next major version of `vue-jest`. Please update your configuration.'
    ),
    '\n'
  )

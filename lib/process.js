...
function processScript (scriptPart, vueJestConfig, filePath, babelOverWrites) {
  if (!scriptPart) {
    return { code: '' }
  }

  if (/^typescript|tsx?$/.test(scriptPart.lang)) {
    return compileTypescript(scriptPart.content, vueJestConfig, filePath, babelOverWrites)
  }

  if (scriptPart.lang === 'coffee' || scriptPart.lang === 'coffeescript') {
    return compileCoffeeScript(scriptPart.content, vueJestConfig, filePath, babelOverWrites)
  }

  return compileBabel(scriptPart.content, undefined, undefined, vueJestConfig, filePath, babelOverWrites)
}

// fourth argument is istanbul, it's injecting automatically
module.exports = function (src, filePath, jestConfig, _, babelOverWrites) {
  ...
  const result = processScript(parts.script, vueJestConfig, filePath, babelOverWrites)

...
function resolveExternalSrcPath(path, filePath, config) {
  return applyModuleNameMapper(path, filePath, config)
}
...
function processScript(scriptPart, filePath, config) {
  ...
  if (scriptPart.src) {
    scriptPart.content = loadSrc(
      resolveExternalSrcPath(scriptPart.src, filePath, config),
      filePath
    )
...
function processTemplate(template, filename, config) {
  ...
  if (template.src) {
    template.content = loadSrc(
      resolveExternalSrcPath(template.src, filename, config),
      filename
    )
  }

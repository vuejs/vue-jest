...
exports.isValidTransformer = function isValidTransformer(transformer = {}) {
  return (
    transformer.process || transformer.postprocess || transformer.preprocess
  )
}
...
const getCustomTransformer = function getCustomTransformer(
  ...
  if (transformerPath) {
    ...
    if (!validTransformer) {
      throwError(
        `transformer must contain at least one of process, preprocess, or ` +
          `postprocess method`
      )
    }
